...
import {IProduct,IProductResponse, ICustomer, IOrder, IState, IPagedResults, ICustomerResponse } from '../shared/interfaces';

@Injectable()
export class DataService {
  
    baseUrl: string = '/api/customers';
    baseStatesUrl: string = '/api/products'
    ...
    getProducts() : Observable<IProduct[]> {
        return this.http.get<IProduct[]>(this.baseStatesUrl)
                   .map((products: IProduct[]) => {//res.json(data.products);
                      // this.calculateCustomersOrderTotal(customers);
                       return products;
                   })
                   .catch(this.handleError);
    ...
    getProduct(id: number) : Observable<IProduct> {
        console.log("get product just started");
        return this.getProducts()
        .map((products: IProduct[]) => products.find(p => p.id === id));
        
       // return this.http.get<IProduct>(this.baseStatesUrl + '/' + id)
        //           .catch(this.handleError);
    }
    ...
    insertProduct(product: IProduct) : Observable<IProduct> {
        return this.http.post<IProductResponse>(this.baseStatesUrl, product)
                   .map((data) => {
    ...
    updateProduct(product: IProduct) : Observable<IProduct> {
        return this.http.put<IProductResponse>(this.baseStatesUrl + '/' + product.id, product) 
                   .map((data) => {
                       console.log('updateProduct status: ' + data.status);
                       return data.product;
                   })
                   .catch(this.handleError);
    }
    ...
    deleteProduct(id: number) : Observable<boolean> {
        return this.http.delete<boolean>(this.baseStatesUrl + '/' + id)
                   .catch(this.handleError);
    }
